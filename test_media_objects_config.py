...
def test_configurable_media_objects():
    ...
    try:
        ...
        config_media = {
            k: v
            for k, v in media_objects.items()
            if not k.startswith(("album_", "sonos_"))
        }
        ...
        logger.info(
            f"✓ Media objects loading enabled: {player.is_media_objects_loading_enabled()}"
        )
        ...
        config_media_after = {
            k: v
            for k, v in media_objects_after.items()
            if not k.startswith(("album_", "sonos_"))
        }

        logger.info(
            f"✓ Media objects after disabling: {len(config_media_after)} from configuration"
        )
        ...
        config_media_final = {
            k: v
            for k, v in media_objects_final.items()
            if not k.startswith(("album_", "sonos_"))
        }

        logger.info(
            f"✓ Media objects after re-enabling: {len(config_media_final)} from configuration"
        )
        ...
        logger.info(
            f"✓ Initial loading state: {config_manager.is_media_objects_loading_enabled()}"
        )
        logger.info(
            f"✓ Initial media objects count: {len(config_manager.get_media_objects())}"
        )

        # Disable loading
        config_manager.set_media_objects_loading(False)
        logger.info(
            f"✓ After disable - loading state: {config_manager.is_media_objects_loading_enabled()}"
        )
        logger.info(
            f"✓ After disable - media objects count: {len(config_manager.get_media_objects())}"
        )

        # Re-enable loading
        config_manager.set_media_objects_loading(True)
        logger.info(
            f"✓ After re-enable - loading state: {config_manager.is_media_objects_loading_enabled()}"
        )
        logger.info(
            f"✓ After re-enable - media objects count: {len(config_manager.get_media_objects())}"
        )
