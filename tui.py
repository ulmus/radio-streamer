...
class RadioStreamerTUI(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Header()
        yield Container(
            self.create_station_panel(), self.create_main_panel(), classes="app-grid"
        )
        yield Footer()
    ...
    def create_station_panel(self) -> Container:
        ...
        for station_id, info in stations.items():
            button = StationButton(station_id, info["name"], id=f"station_{station_id}")
            self.station_buttons[station_id] = button
            buttons.append(button)

        return Container(
            Label("📻 Radio Stations:"), *buttons, classes="station-buttons"
        )
    ...
    def update_status(self) -> None:
        ...
        if self.status_panel:
            # Get current media name for display
            current_media_name = "None"
            if status.current_media:
                current_media_name = status.current_media.name
            self.status_panel.current_station = current_media_name
        ...
        for station_id, button in self.station_buttons.items():
            is_current = status.current_media and status.current_media.id == station_id
    ...
    def play_station(self, station_id: str) -> None:
        ...
        if self.media_player.play_media(station_id):
