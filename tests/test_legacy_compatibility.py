...
class TestSpotifyIntegration:
    """Tests converted from test_spotify_integration.py"""
    
    def test_spotify_models_removed(self):
        """Test that Spotify models are no longer available"""
        # Spotify integration was removed, so these should not be available
        with pytest.raises(ImportError):
            from media_player import SpotifyAlbum, SpotifyTrack
    
    @patch('media_player.VLC_AVAILABLE', True)
    @patch('media.media_player.vlc')
    def test_media_player_spotify_methods_disabled(self, mock_vlc, temp_config_file, temp_music_folder):
...
class TestNowPlayingButton:
    ...
    @patch('media_player.VLC_AVAILABLE', True)
    @patch('media.media_player.vlc')
    def test_now_playing_button_logic(self, mock_vlc, temp_config_file, temp_music_folder):
...
class TestOverlayBaking:
    ...
    def test_overlay_image_creation_logic(self):
        ...
        try:
            ...
            overlay_image = Image.new("RGBA", test_image_size, overlay_color + (128,))  # Semi-transparent
...
class TestStreamDeckAbbeyRoad:
    ...
    @patch('media_player.VLC_AVAILABLE', True)
    @patch('media.media_player.vlc')
    def test_abbey_road_media_object_structure(self, mock_vlc, temp_config_file, temp_music_folder):
